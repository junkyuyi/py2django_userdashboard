...
class UserManager(models.Manager):
	...
	def add(self, request, level):
		...
		if len(messages.get_messages(request)) > 0:
			...
			hashed = bcrypt.hashpw(request.POST['password1'].encode(encoding="utf-8", errors="strict"), bcrypt.gensalt())
	...
	def login(self, request):
		...
		if self.checkEmptyFields(request,					# check for empty fields on request.POST
			...
			try:
				...
				if not bcrypt.hashpw(request.POST['password'].encode(encoding="utf-8", errors="strict"), hashed) == hashed:
					messages.info(request, "Wrong password! Try again!", extra_tags="gen")
			except ObjectDoesNotExist:
